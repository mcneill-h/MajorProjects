...
EPS_DECAY = 4000 ## the higher is the number, the slower is the decay of EPS
...
def plot_durations(show_result=False):
    ...
    if show_result:
        ...
        plt.clf()
        plt.title('Training...')
    plt.xlabel('Episode (average of last 20 games)')
    plt.ylabel('Win rate (%)')
...
#         plt.plot(means.numpy())
    ...
    ##MODIFY EVERYTHING THAT IS HERE
    ##mean of the last 500 numbers
    window = 25
    means = []
...
def game(player_sum, dealer_sum, action): 
    ...
    if str(action)== "tensor([[1]])":
        ...
        if (player_sum <= 21): ##still in play  
            ...
            return (player_sum, dealer_sum) , -2, True, False  
